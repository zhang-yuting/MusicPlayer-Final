...
import com.example.xukefeng.musicplayer.utils.FileUtils;
import com.example.xukefeng.musicplayer.utils.SdCardUtils;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int check = checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE);
            ...
            if (check != PackageManager.PERMISSION_GRANTED) {
                //申请权限
                ActivityCompat.requestPermissions(this , new String[] {Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE} ,STORGE_REQUEST);
            } else {
            //已有权限的情况下可以直接初始化程序
//            init();
            getAssetsList();//获取assets文件
        }
            ...
            if (fileNames.length > 0){
                ...
                for (int i = 0 ; i < fileNames.length ; i++) {
                    if (fileNames[i].endsWith(".mp3")){
                        ...
                        InputStream openSize = getAssets().open(fileNames[i]);
                        int fileSize = openSize.available();

                        //将数据装载到List<MusicInfo>中
                        musicInfo.setMusicName(fileNames[i]);
                        musicInfo.setData("android.resource://package_name/assets/" + fileNames[i]);
                        musicInfo.setSize(fileSize);
                        ...
                        Float size = (float) (fileSize * 1.0 / 1024 / 1024)  ;
    ...
    private void init() {
        //文件路径
        FileUtils.getInstance(this).copyAssetsToSD("music",SdCardUtils.getSDCardBaseDir() + File.separator + "Music");
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        //判断是不是.mp3格式
        /*if (musicInfos.get(position).getMusicName()){
